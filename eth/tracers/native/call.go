...
type sstoreOp struct {
	...
	OldValue hexutility.Bytes `json:"old"`
	NewValue hexutility.Bytes `json:"new"`
}

func newKeccak256Op(in []byte, out []byte) *keccak256Op {
...
func newSstoreOp(offset [32]byte, oldVal [32]byte, newVal [32]byte) *sstoreOp {
	return &sstoreOp{
		baseOp{vm.SSTORE},
		offset[:],
		oldVal[:],
...
func (t *callTracer) CaptureState(pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, err error) {
	...
	case vm.SSTORE:
		if stackSize >= 2 {
			...
			op := newSstoreOp(offset.Bytes32(), oldValue.Bytes32(), value.Bytes32())
