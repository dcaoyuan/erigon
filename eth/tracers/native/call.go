...
type baseOp struct {
	OpSn uint16    `json:"opsn,omitempty"`
...
type sstoreOp struct {
	...
	OldValue hexutility.Bytes `json:"oldVal"`
	NewValue hexutility.Bytes `json:"newVal"`
}

func newKeccak256Op(opsn uint16, in []byte, out []byte) *keccak256Op {
...
func newSstoreOp(opsn uint16, offset [32]byte, oldVal [32]byte, newVal [32]byte) *sstoreOp {
	return &sstoreOp{
...
type callFrame struct {
	...
	OpSn     uint16            `json:"opsn,omitempty" rlp:"optional"`
...
func (t *callTracer) getNextOpSn() uint16 {
	t.nextOpSn++
